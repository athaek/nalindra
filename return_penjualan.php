	...
	<div class="padding">
		<div class="bgwhite">
			<div class="padding">
				...
				<form class="form-input" method="post" action="handler.php?action=tambah_tempo_return_penjualan" style="padding-top: 30px;">
		...
		<div class="bgwhite">
			...
			<tbody id="contenth">
				...
				$data=$root->con->query("select barang.nama_barang,tempo_return_penjualan.id_subreturnpenj,tempo_return_penjualan.id_barang,tempo_return_penjualan.jumlah_beli,tempo_return_penjualan.total_harga,tempo_return_penjualan.diskon from tempo_return_penjualan inner join barang on barang.id_barang=tempo_return_penjualan.id_barang where trx='$trx'");
				$getsum=$root->con->query("select sum(total_harga) as grand_total from tempo_return_penjualan where trx='$trx'");
				...
				while ($f=$data->fetch_assoc()) {
					?><tr>
						...
						<td><a href="handler.php?action=hapus_tempo_return_penj&id_tempo_return_penj=<?= $f['id_subreturnpenj'] ?>&id_barang=<?= $f['id_barang'] ?>&jumbel=<?= $f['jumlah_beli'] ?>" class="btn redtbl"><span class="btn-hapus-tooltip">Cancel</span><i class="fa fa-close"></i></a></td>
						</tr>
					<?php
				}
				?>
			...
			<br>
			<form class="form-input" action="handler.php?action=selesai_return" method="post">
