	...
	<div class="padding">
		<div class="bgwhite">
			<div class="padding">
				...
				<form class="form-input" method="post" action="handler.php?action=tambah_tempo_return_pembelian" style="padding-top: 30px;">
		...
		<div class="bgwhite">
			...
			<tbody id="contenth">
				...
				$data=$root->con->query("select barang.nama_barang,tempo_return_pembelian.id_subreturnpem,tempo_return_pembelian.id_barang,tempo_return_pembelian.jumlah_beli,tempo_return_pembelian.total_harga from tempo_return_pembelian inner join barang on barang.id_barang=tempo_return_pembelian.id_barang where trx='$trx'");
				$getsum=$root->con->query("select sum(total_harga) as grand_total from tempo_return_pembelian where trx='$trx'");
				...
				while ($f=$data->fetch_assoc()) {
					?><tr>
						...
						<td><a href="handler.php?action=hapus_tempo_return_pem&id_tempo_return_pem=<?= $f['id_subreturnpem'] ?>&id_barang=<?= $f['id_barang'] ?>&jumbel=<?= $f['jumlah_beli'] ?>" class="btn redtbl"><span class="btn-hapus-tooltip">Cancel</span><i class="fa fa-close"></i></a></td>
						</tr>
					<?php
				}
				?>
			...
			<br>
			<form class="form-input" action="handler.php?action=selesai_return_pembelian" method="post">
