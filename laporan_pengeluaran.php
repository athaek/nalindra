...
<?php
	if (isset($_GET['action']) && $_GET['action']=="detail_pengeluaran") {
		include "detail_pengeluaran.php";
	}elseif(isset($_GET['action']) && $_GET['action']=="chart") {
		include "chart.php";
	}
	else{
?>
...
<div class="content">
	<div class="padding">
		<div class="bgwhite">
			...
			<div class="contenttop">
				<div class="left">
					<h3 class="jdl">Laporan Pengeluaran</h3>
				</div>
				<div class="right">
					<script type="text/javascript">
						function gotojenis(val){
							var value=val.options[val.selectedIndex].value;
							window.location.href="laporan_pengeluaran.php?jenis="+value+"";
						}
						function gotofilter(val){
							var value=val.options[val.selectedIndex].value;
							
							window.location.href="laporan_pengeluaran.php?jenis=<?php if (isset($_GET['jenis'])) {
								echo $_GET['jenis'];
							} ?>&filter_record="+value;
						}
					</script>
    ...
    <form action="cetak_laporan_pengeluaran.php" style="display: inline;" target="_blank" method="post">
					...
					<select class="leftin1" onchange="gotofilter(this)" required="required" name="tgl_laporan">
						<?php
							if (isset($_GET['jenis'])&&$_GET['jenis']=='perhari') {
								...
								<?php
								$data=$root->con->query("select distinct date(tgl_pengeluaran) as tgl_pengeluaran from pengeluaran order by id_pengeluaran desc");
								while ($f=$data->fetch_assoc()) {
									?>
										<option <?php if (isset($_GET['filter_record'])) { if ($_GET['filter_record'] == date('d-m-Y',strtotime($f['tgl_pengeluaran']))) { echo "selected"; } } ?> value="<?= date('d-m-Y',strtotime($f['tgl_pengeluaran'])) ?>"><?= date('d-m-Y',strtotime($f['tgl_pengeluaran'])) ?></option>
									<?php
								}
						...
						<?php
							$data=$root->con->query("select distinct EXTRACT(YEAR FROM tgl_pengeluaran) AS OrderYear,EXTRACT(MONTH FROM tgl_pengeluaran) AS OrderMonth from pengeluaran order by id_pengeluaran desc");
							while ($f=$data->fetch_assoc()) {
								?>
					...
					<a href="?action=chart" class="btnblue" style="background: #f33155"><i class="fa fa-mail-reply"></i>Grafik</a>
					</form>
				</div>
				<div class="both"></div>
			</div>
			...
			<tbody>
					...
					if (isset($_GET['filter_record'])) {
						...
						$root->filter_tampil_laporan_pengeluaran($_GET['filter_record'],$aksi1);
					}else{
					$root->tampil_laporan_pengeluaran();
					}
					?>
</tbody>
